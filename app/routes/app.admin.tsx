...
const processInventoryRules = async (admin: any, variantIds: string[], pricingRule?: any) => {
  const results: Array<{
    variantId: string;
    success: boolean;
    oldPrice?: string;
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  if (actionType === "inventory_rules") {
    ...
    return await processInventoryRules(admin, variantIds, pricingRule);
  }
...
export default function Admin() {
  ...
  return (
    <Page>
      <TitleBar title="Bulk Price Management" />
      <BlockStack gap="500">
        <Layout>
          <Layout.Section>
            <Card>
